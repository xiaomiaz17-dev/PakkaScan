    ...
    <div style={{ background: style.bg, border: "2px solid " + style.border, borderRadius: "16px", padding: "24px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "20px", flexWrap: "wrap" }}>
      ...
      <div style={{ flex: 1, minWidth: "200px" }}>
        ...
        {urduLabelFor(style.label) && (
          <div className={nastaliq.className} dir="rtl" style={{ fontSize: "22px", color: style.color, opacity: 0.85, marginBottom: "8px", fontWeight: 700, lineHeight: 1.4, maxWidth: "100%", overflowWrap: "anywhere", wordBreak: "break-word" }}>{urduLabelFor(style.label)}</div>
        )}
          ...
          <div ref={resultsRef} style={{ width: "100%", paddingTop: "24px", marginTop: "32px", backgroundColor: "#ffffff", padding: "28px", borderRadius: "16px", border: "1px solid #e2e8f0", boxSizing: "border-box", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.08)" }}>
            ...
            <div style={{ marginBottom: "8px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
                <h3 style={{ fontSize: "22px", fontWeight: 800, color: "#0f172a", margin: "0 0 4px 0" }}>{reportTitleFor(results.tier, (results.documents?.[0] as any)?.classification?.documentType || (results.documents?.[0] as any)?.documentType, (results.documents || []).flatMap((d: any) => [d?.classification?.documentType || d?.documentType || d?.type || "", d?.fileName || d?.filename || ""]))}</h3>
                {results.referenceCode && (
            ...
            {isMultiDoc && combinedVerdict ? (
              <CombinedVerdictHero combined={{...combinedVerdict, verdict: (/TENANCY|LEASE|RENTAL/i.test(String((results?.documents || []).map((d: any) => d?.classification?.documentType || "").join(" "))) && String(combinedVerdict.verdict || "").includes("CAUTION")) ? combinedVerdict.verdict : (results?.riskLabel === "CRITICAL" ? "DO_NOT_PROCEED" : ((_flaggedHi || _hasStampFactor || results?.riskLabel === "HIGH") && String(combinedVerdict.verdict).includes("PROCEED") && !String(combinedVerdict.verdict).includes("CAUTION") && !String(combinedVerdict.verdict).includes("DO NOT") ? "PROCEED WITH CAUTION" : combinedVerdict.verdict)), posture: (/TENANCY|LEASE|RENTAL/i.test(String((results?.documents || []).map((d: any) => d?.classification?.documentType || "").join(" "))) && String(combinedVerdict.verdict || "").includes("CAUTION")) ? "CAUTIOUS" : (results?.riskLabel === "CRITICAL" ? "DO_NOT_PROCEED" : ((_flaggedHi || _hasStampFactor || results?.riskLabel === "HIGH") ? "CAUTIOUS" : combinedVerdict.posture))}} docCount={results?.documents?.length ?? 0} urduReasoning={urduTranslations["combinedReasoning"]} />
              ...
              <div style={{ flex: "1 1 240px", minWidth: 0 }}>
                ...
                <div style={{ marginTop: "14px", display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        let hdr = res.headers.get("X-PakkaScan-Pdf-Hash") || res.headers.get("x-pakkascan-pdf-hash") || res.headers.get("ETag") || res.headers.get("etag");
                        if (hdr) hdr = hdr.replace(/"/g, "").trim();
                        const blob = await res.blob();
                        // F6: always derive hash from the blob so it is visible even if response headers are stripped
                        let clientHash = "";
                        try {
                  ...
                  <div style={{ marginTop: 10, padding: "10px 12px", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 12, maxWidth: 420 }}>
                    <div style={{ fontWeight: 700, color: "#0f172a", marginBottom: 4 }}>PDF authenticity hash (SHA-256)</div>
                    {pdfHashForCopy ? (
                      <>
                      ...
                      <div style={{ color: "#64748b", lineHeight: 1.45 }}>
                        Click <strong>Download PDF Passport</strong> above. The SHA-256 hash will appear here so anyone can confirm the file is unaltered on the verify page.
                      </div>
                    )}
                  </div>
                </div>
